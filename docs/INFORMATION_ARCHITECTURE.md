...
- **A Community Centre search is the one exception to what the Location Pill displays.** It still resolves as a location intent and still scopes/filters results the same way a Neighbourhood, Municipality, or Postal Code search does — that part of Search Context is unaffected. But the Pill itself only ever shows area/proximity context (`Near you`, a neighbourhood, a municipality, a postal code) — never a specific facility name. A named building is surfaced in result cards and the Quick Action Sheet, not the global header. See `docs/SEARCH_ENGINE.md`'s Location Override vs. Persistent "Near You" section for the exact mechanics.
...
- 2026-08-28 — Added the Community Centre exception to Location Context, per Round 2 physical-iPhone QA: a facility search was found to replace the Pill's "Near me" text with the full Community Centre name, which this document's own example list for the persistent context (`Near you`/neighbourhood/municipality/postal code) never actually included — the implementation had followed `docs/SEARCH_ENGINE.md`'s literal location-intent rule further than this document's own framing anticipated. Fixed at the display layer only (`app/page.tsx`'s `locationPillLabel()`); Community Centre's role in result filtering is unchanged. Full mechanics recorded in `docs/SEARCH_ENGINE.md`.
