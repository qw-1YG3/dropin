...
## 14. Open Issue — Aurora ActiveCommunities Data Completeness (Non-Blocking)

**Status: unresolved, tracked, non-blocking.** Discovered during the first real Phase 5B-3 production refresh run (`docs/PHASE_5B3_DAILY_REFRESH_SCHEDULER.md`), diagnosed the same day. Recorded here — this project's established backlog/open-items location (alongside §10, §12) — rather than left implicit in a phase-specific document only, per this project's own "don't rediscover from scratch" discipline. **Not implemented, not worked around, not fixed here.**
...
**Not scheduled, not assigned a phase yet.** A dedicated future investigation (mirroring Phase 3.6A/B's own original methodology) would be the appropriate next step if/when this is prioritized.
